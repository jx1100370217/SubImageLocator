"""SubImageLocator - Gradio WebUI for sub-image localization."""
...
from matchers.feature_matcher import match_features, preload_models
    ...
    with gr.Blocks(title="SubImageLocator - 子图定位") as app:
        ...
        <div style="text-align: center; padding: 20px;">
            ...
            <p style="font-size: 16px; color: #666;">
                ...
                基于 <b>SuperPoint + LightGlue</b> 特征匹配，模型预加载，响应 &lt;50ms
            </p>
        </div>
        """)
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                with gr.Accordion("⚙️ 高级设置", open=False):
                    confidence_threshold = gr.Slider(0.0, 1.0, value=0.3, step=0.05, label="置信度阈值")
                    min_feature_matches = gr.Slider(4, 50, value=8, step=1, label="最小特征匹配数")
...
def main():
    print("=" * 60, flush=True)
    print("SubImageLocator starting...", flush=True)

    print("[init] Generating examples...", flush=True)
    generate_examples()

    print("[init] Preloading SuperPoint+LightGlue...", flush=True)
    try:
        preload_models()
